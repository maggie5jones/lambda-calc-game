...
func move(dir):
	...
	if !ray.is_colliding():
		...
		if collider.is_in_group('variableX') or \
			collider.is_in_group('variableY') or \
			collider.is_in_group('variableZ') or \
			collider.is_in_group('function') or \
			collider.is_in_group('bool'):
